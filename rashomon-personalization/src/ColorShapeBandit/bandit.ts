import { configurationUniverse, type Configuration } from "./configuration.ts";
...
export function initializeContext(): UserContext {
  ...
  return {
    ...
    sigma2: new Array(totalNumberOfFeatures).fill(1),
    beta: 1,
  };
}
...
export function updateContext(
  ...
  const encoding = encode(config, configurationUniverse);
  return {
    mu: updateMu(context.mu, context.sigma2, encoding, reward),
    sigma2: updateSigma2(context.mu, context.sigma2, encoding, reward),
    beta: context.beta,
  };
}
...
export function sampleConfig(context: UserContext): Configuration {
  const sampledMu = sampleMultiVariateNormal(context.mu, context.sigma2);
  ...
  ].map((o, idx) => o + argMax(sampledMu.slice(o, cumulativeOffsets[idx])));
  const encoding = assembleFeatureEncoding(
    cumulativeOffsets[cumulativeOffsets.length - 1],
    absoluteConfigurationPositions,
  );
  return decode(encoding, configurationUniverse);
}
