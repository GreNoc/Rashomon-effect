...
@dataclass
class Config:
    data_path: str = "ebm_model/bike.csv"
    ...
    )  # ['mnth', 'hr', 'temp', 'atemp', 'hum', 'windspeed', 'weekday']
...
def add_plot_data(
    ...
    if feat_name in ["num__hr", "num__windspeed", "num__atemp"]:
...
def load_data() -> pd.DataFrame:
    ...
    # Remove yr as we remove it as feature later and hence, it increases the performance.
    df = df[df["yr"] != 0]

    # Re-scale the 'temp' and 'atemp' columns
...
def debug_interaction_issue(ebm: ExplainableBoostingRegressor):
    ...
    for feat in feat_list:
        if not "&" in feat:
            ...
            if term1 in feat_list and term2 in feat_list:
                continue
            else:
                print(feat_list)
                raise ValueError("Interaction should not exist.")
...
def train_model(
    ...
    for i, params in enumerate(param_grid_dict):
        ...
        # Train model
        # manual_params = {"max_bins": 256, "min_samples_leaf": 64, "interactions": 2}
        model_params = {
            param_name: param_value
            for (param_name, param_value) in params.items()
            if param_name != "monotonicity_constraints" and param_name != "exclude"
        }

        feature_names = [
            feat for feat in ct.get_feature_names_out() if feat not in params["exclude"]
        ]
        ...
        score = ebm.score(X_test_selected, y_test)
        logger.debug(f"R^2 score: {score:.6f} Params: {params}")

        debug_interaction_issue(ebm)
...
def create_plots(ebm: ExplainableBoostingRegressor, model_path: str | os.PathLike):
    ...
    for index, feat_name in enumerate(ebm_global.data()["names"]):
        ...
        # Is numerical:
            ...
            # "Manual" manipulation as we do not use the intercept for simplicity reasons.
            ...
            add_plot_data(
                ...
                x_min=x_min,
                x_max=x_max,
                x_labels=x_labels if feat_name == "num__weekday" else None,
            )
