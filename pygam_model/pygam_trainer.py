...
@dataclass
class Config:
    data_path: str = "https://raw.githubusercontent.com/stedy/Machine-Learning-with-R-datasets/master/insurance.csv"
    ...
    target: str = field(default_factory=lambda: "charges")
    test_size: float = 0.3
    random_state: int = 42
    ...
    name_mapping: dict = field(
        default_factory=lambda: {
            # Numerische Feature-Mappings
            "num__age": "Age",
            "num__bmi": "BMI",
            "num__children": "Children",
            
            # Kategoriale Feature-Mappings (OrdinalEncoder nutzt die Ursprungsnamen mit Präfix)
            "cat__sex": "Sex",
            "cat__smoker": "Smoker",
            "cat__region": "Region",
            
            # Interaktionen (falls du später in den parameters "interactions" einfügst)
            "num__bmi & cat__smoker": "BMI x Smoker",
            "num__age & cat__smoker": "Age x Smoker",
            
            # Y-Achsen-Beschriftung für die Plots
            "Effect on Prediction": "Effect on Charges",
        }
    )
...
def train_model(
    ...
    # 1. PyGAM Formel dynamisch aufbauen
    terms = None
    
    # Numerische Features (Splines: s)
    num_count = len(config.numerical_cols)
    ...
    # Kategoriale Features (Factors: f)
    cat_count = len(config.categorical_cols)
    for i in range(cat_count):
        idx = num_count + i  # Indizes laufen nach den numerischen weiter
        if terms is None:
            terms = f(idx)
        else:
            terms += f(idx)

    # (Optional) Interaktionen (Tensor-Produkte: te), falls in der Config definiert
    # Beispielhaft, wenn config.parameters "interactions" als Liste von Tupeln [(0,1)] enthält
    # for mod_tuple in manual_params.get("interactions", []):
    #     terms += te(mod_tuple[0], mod_tuple[1])

    # 2. PyGAM Modell trainieren
    gam = LinearGAM(terms)
    gam.fit(X_train, y_train)
    
    # 3. Model Scoring
    # Bei PyGAM nutzen wir pseudo_r2 zur Bewertung der Varianzaufklärung
    score = gam.statistics_['pseudo_r2']['explained_deviance']
    logger.debug(f"PyGAM Pseudo R^2 score: {score:.6f}")
...
def main() -> None:
    ...
    scores_df.to_csv("medical_scores.csv", index=False)
    scores_df.to_json("plot_data_medical_insurance.json", orient="records")
