...
EXPERIMENT_START = berlin_tz.localize(datetime(2024, 11, 10, 19, 36))
EXPERIMENT_END = berlin_tz.localize(datetime(2024, 11, 10, 22, 59))
...
engine = create_engine("sqlite:///analytics_store_exports/1731310121.db")
...
with Session(engine) as session:
    experiment_query = select(AnalyticsEvents).where(
        ...
        AnalyticsEvents.experimentTag == "managementInsights4",
    )
    ...
    management_insights_report.to_excel("managementInsights4_experiment_results.xlsx")
    ...
    with open("managementInsights4_personalization_context.json", "w") as fp:
