...
EXPERIMENT_START = berlin_tz.localize(datetime(2024, 11, 8, 20, 45))
EXPERIMENT_END = berlin_tz.localize(datetime(2024, 11, 8, 21, 56))
...
engine = create_engine("sqlite:///analytics_store_exports/1731148404.db")
...
with Session(engine) as session:
    experiment_query = select(AnalyticsEvents).where(
        ...
        AnalyticsEvents.experimentTag == "managementInsights3",
    )
    ...
    management_insights_report.to_excel("managementInsights3_experiment_results.xlsx")
    ...
    with open("managementInsight3_personalization_context.json", "w") as fp:
